!!! 部分知识点来源于：https://mp.weixin.qq.com/s/sFUvgaQUXSA8b4hshhbwOQ !!!
...
![图片](G:\knowledge\消息队列\images\消费者和分区.webp)
...
>
> <img src="G:\knowledge\消息队列\images\Range.webp" alt="图片" style="zoom:50%;" />
>
> 但是这个分配策略会有点小问题，他是根据主题进行分配，所以如果消费者组订阅了多个主题，那就有可能导致分区分配不均衡。
>
> 比如下图中两个主题的P0\P1都被分配给了A，这样A有4个分区，而B只有2个，如果这样的主题数量越多，那么不均衡就越严重。
>
> <img src="G:\knowledge\消息队列\images\Range不均衡.webp" alt="图片" style="zoom:50%;" />
...
>P0->A，P1->B，P1->A。。。以此类推
>
><img src="G:\knowledge\消息队列\images\RoundRobin.webp" alt="图片" style="zoom:50%;" />
...
![](G:\knowledge\消息队列\images\HW&LEO-2.png)
...
<img src="G:\knowledge\消息队列\images\副本同步1.webp" style="zoom:50%;" />

2. 消息还在继续写入，Leader的LEO值又发生了变化，两个Follower也各自拉取到了自己的消息，于是更新自己的LEO值，但是这时候Leader的HW依然没有改变。

<img src="G:\knowledge\消息队列\images\副本同步2.webp" style="zoom:50%;" />

3. Follower再次向Leader拉取数据，这时候Leader会更新自己的HW值，取Follower中的最小的LEO值来更新。

<img src="G:\knowledge\消息队列\images\副本同步3.webp" style="zoom:50%;" />

4. Leader响应自己的HW给Follower，Follower更新自己的HW值，因为又拉取到了消息，所以再次更新LEO，流程以此类推。

<img src="G:\knowledge\消息队列\images\副本同步4.webp" style="zoom:50%;" />
