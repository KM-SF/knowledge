...
+ 模板：对于需要从左子树获得信息（left），从右子树获得信息（right），然后将left和right信息整合在一起的可以使用一下模板
...
+ Morris遍历：需要空间复杂度为O（1）时，需要用这个遍历![](/算法/images/二叉树Morris遍历.png)
...
+ 力扣：[面试题 04.06. 后继者](https://leetcode-cn.com/problems/successor-lcci/)
...
+ 题解：
  ...
  + 因为完成二叉树是除了最深层其他层数都时满二叉树，而最深层的所有节点得靠左
  + 对于完全二叉树而言：就有数组的大小（总节点个数-1）等于最大的下标。所以如果是完成二叉树则每个节点的索引等于到该节点的总数-1
      ...
      bool isCompleteTree(TreeNode* root) {
          ...
          while (!que.empty()) {
              ...
              while (nSize--) {
                  ...
                  que.pop();
                  // 如果到该节点总数量-1不等于该节点索引，则认为不是完全二叉树
                  if (cnt - 1 != node.second) return false;
                  // 插入左右子节点和其索引
                  if (node.first->left) que.push({node.first->left, 2*node.second +1});
                  if (node.first->right) que.push({node.first->right, 2*node.second +2});
              }
          }
          return true;
      }
